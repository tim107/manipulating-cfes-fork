...
neg_not_pro = negative_not_protected_indices(data_v, model, protected_v)
final_preds = (model(data_v[neg_not_pro] + noise)[:,0] > 0.5 ).int()
succ = (torch.sum(torch.abs(model(data_v[neg_not_pro] + noise)[:,0] > 0.5)) / torch.sum(neg_not_pro))
print ("Delta flip success", succ)
final_preds = (model(data_v)[:,0] > 0.5).int() 
print ("Validation Accuracy", torch.sum(final_preds == labels_v) / final_preds.shape[0])
print ('#######')
...
if CFNAME == "revise":
    ...
    val_data_loader = torch.utils.data.DataLoader(VaeDataset(data_v, labels_v), batch_size=100, shuffle=False)
...
if RUN_SECOND:
    ...
    for e in tqdm(range(RUNE)):
        ...
        # with torch.no_grad():
        predictions = model(data_v)[:,0]
        arg_preds = (predictions >= 0.5).int()
        test_acc = (torch.sum(arg_preds == labels_v).float() / labels_v.shape[0])
        ...
        te_protected_negative, te_nonprotected_negative = get_groups(data_v, model(data_v)[:,0], protected_v)
        ...
        cost_ratio = -training_diff['Testing_Expected_CF_Burden_Not_Protected'] / training_diff['Perturbed_Burden']
        disparity = training_diff["Testing_Delta"]
        current_val_loss = cost_ratio + disparity
        if current_val_loss < lowest_val_loss:
            lowest_val_loss = current_val_loss
            val_patience = 2
            if args.save_model:
