...
def plot_decision_boundary(pred_func, X, y, protected, writer=None, cfs=None, cfs_perturbed=None, e=None):
    ...
    plt.scatter(X[:, 0][protected == NOT_PROTECTED], X[:, 1][protected == NOT_PROTECTED],
                c=y[protected == NOT_PROTECTED], marker='+', cmap=plt.cm.binary)
...
def assess(e, model, data, protected, labels, data_t, protected_t, labels_t, cf_name, cf_args, writer, noise, WRITER,
           df, verbose=True, r=False, savelof=False):
    ...
    def get_cf(cur_data, model, exp):
        ...
        for _ in range(20):
            dice_exp = exp.generate_counterfactuals(cur_data.detach().cpu().numpy().tolist(),
                                                    ...
                                                    proximity_weight=starting_weight, total_CFs=4, verbose=False,
                                                    desired_class=1, min_iter=0, max_iter=100)
...
def wachter(model, data, positive_data, lmbda, target, cat_features, mad, alglr=1e-1, eps=1e-10, use_tqdm=False,
            sparse=False, opt='sgd', exit_on_greater=False):
    if not sparse:
...
def revise(model, cf_instance, data, positive_data, lmbda, target, cat_features, features_to_vary, alglr=1e-1, eps=1e-10,
    ...
    def get_cf(x, features_to_vary, target=0.7):
        return cf_instance.generate_counterfactuals(x, features_to_vary, target, feature_weights=None,
